...
function fit(data: Data, config: Config): Data {
  ...
  for (let i = 0; i < config.generationAmt; i++) {
    ...
    let parents = select(population, config)
...
function getGeneration(data: Data, yEntryPoints: Map<string, number>[], config: Config): Child[] {
  let population: Child[] = []
  // Compute new generation
  for (let i = 0; i < config.populationSize; i++) {
    let entryPoints = yEntryPoints ? yEntryPoints[i] : undefined
    let result: [XLayer[], GenePool] = visit(data, entryPoints)
    let loss = getLoss(result, config)
